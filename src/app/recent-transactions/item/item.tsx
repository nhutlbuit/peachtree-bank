...
function Item() {
    ...
    const getImageName1 = async (column: any): Promise<string> => {
        let merchantName = column?.merchant?.name.toLowerCase().split(' ').join('-');
        let imageObj = await import(`../../../assets/icons/${merchantName}.png`);
        let imageName = imageObj.default;
        return imageName;
    }
    ...
    return (
        <>
            <table>
                <tbody>
                    {data.map((column: any, key: number) => (
                        <tr key={key}>
                            ...
                            <td style={{ fontWeight: 'bold', fontSize: '16px' }}>${column?.transaction?.amountCurrency?.amount}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
